...
export function registerCommands(program) {
    ...
    program
        ...
        .action((name) => {
        printHeaderOnce();
        console.log(boxen(chalk.yellow(`linex show ${name}`) +
            "\n\n" +
            chalk.gray("Show detailed metadata and properties for a registered object."), { borderColor: "yellow", padding: 1 }));
        const meta = get(name);
        if (!meta) {
    ...
    program
        ...
        .action(() => {
        printHeaderOnce();
        console.log(boxen(chalk.yellow("linex deps") +
            "\n\n" +
            chalk.gray("Visualize the dependency graph of all registered objects."), { borderColor: "yellow", padding: 1 }));
        const registry = LinexRegistry.getInstance();
        const graphStr = registry["dependencyGraph"].visualize();
        console.log(boxen(chalk.magenta(graphStr), { borderColor: "magenta", padding: 1 }));
    });
    program
    ...
    program
        ...
        registry.saveToFile();
        // 3. 디버깅 로그 (필요시)
        console.log("edges", Array.from(registry["dependencyGraph"]["edges"].entries()));
        console.log("nodes", Array.from(registry["dependencyGraph"]["nodes"].keys()));
        // 4. 안내 메시지
